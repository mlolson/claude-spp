 ...
 */
export function detectConcepts(code: string, language: string): string[] {
  const concepts: string[] = [];

  // TypeScript/JavaScript patterns
  if (language === "typescript" || language === "javascript") {
    if (/async\s+(function|\(|[a-zA-Z])/.test(code)) concepts.push("async/await");
    if (/await\s+/.test(code)) concepts.push("async/await");
    if (/new\s+Promise/.test(code)) concepts.push("promises");
    if (/\.then\s*\(/.test(code)) concepts.push("promises");
    if (/try\s*{/.test(code)) concepts.push("error handling");
    if (/catch\s*\(/.test(code)) concepts.push("error handling");
    if (/interface\s+\w+/.test(code)) concepts.push("interfaces");
    if (/type\s+\w+\s*=/.test(code)) concepts.push("type aliases");
    if (/<\w+>/.test(code)) concepts.push("generics");
    if (/class\s+\w+/.test(code)) concepts.push("classes");
    if (/extends\s+\w+/.test(code)) concepts.push("inheritance");
    if (/implements\s+\w+/.test(code)) concepts.push("interfaces");
    if (/\.(map|filter|reduce|forEach)\s*\(/.test(code)) concepts.push("array methods");
    if (/=>\s*{?/.test(code)) concepts.push("arrow functions");
    if (/\.\.\.\w+/.test(code)) concepts.push("spread operator");
    if (/const\s*\[/.test(code) || /const\s*{/.test(code)) concepts.push("destructuring");
    if (/import\s+/.test(code)) concepts.push("modules");
    if (/export\s+/.test(code)) concepts.push("modules");
    if (/useState|useEffect|useCallback|useMemo/.test(code)) concepts.push("React hooks");
    if (/useRef|useContext|useReducer/.test(code)) concepts.push("React hooks");
  }

  // Python patterns
  if (language === "python") {
    if (/def\s+\w+/.test(code)) concepts.push("functions");
    if (/class\s+\w+/.test(code)) concepts.push("classes");
    if (/async\s+def/.test(code)) concepts.push("async/await");
    if (/await\s+/.test(code)) concepts.push("async/await");
    if (/\[.*for.*in.*\]/.test(code)) concepts.push("list comprehensions");
    if (/{.*for.*in.*}/.test(code)) concepts.push("dict comprehensions");
    if (/with\s+/.test(code)) concepts.push("context managers");
    if (/@\w+/.test(code)) concepts.push("decorators");
    if (/try:/.test(code)) concepts.push("error handling");
    if (/except\s*:?/.test(code)) concepts.push("error handling");
  }
 ...
 */
const CONCEPT_QUESTIONS: Record<string, Array<{ question: string; hint: string }>> = {
  "async/await": [
    {
 ...
 */
const GENERIC_QUESTIONS: Array<{ question: string; hint: string }> = [
  {
 ...
 */
export function allQuestionsAnswered(checkpoint: ReviewCheckpoint): boolean {
 ...
 */
export function formatCheckpointForReview(checkpoint: ReviewCheckpoint): string {
  const lines: string[] = [
...
export function requiresReview(
  filePath: string,
  addedLines: string[],
  config: { minLinesForReview?: number; excludePatterns?: string[] } = {}
): boolean {
 ...
 */
export function addCheckpointToSession(
 ...
 */
export function completeCheckpointInSession(
  session: ReviewSession,
  checkpointId: string,
  ...
  const updatedCheckpoint = updateCheckpointStatus(
 ...
 */
export function getReviewCompletionRate(session: ReviewSession): number {
  if (session.totalReviews === 0) return 1;
