 ...
 */
const SOCRATIC_QUESTIONS: Record<QuestionType, string[]> = {
  conceptual: [
 ...
 */
const OPENERS: Record<EscalationLevel, string[]> = {
  socratic: [
 ...
 */
const DOC_POINTERS: Record<string, string> = {
 ...
 */
export function getTeachingTemplate(context: TemplateContext): TeachingTemplate {
...
export function getEscalationLevel(
  ...
  if (classification.type === "debugging") {
    if (attemptCount <= 1) return "guided";
    if (attemptCount <= 2) return "hints";
    if (attemptCount <= 3) return "explanation";
    return "direct";
  }

  // Standard escalation path
  if (attemptCount <= 1) return "socratic";
  if (attemptCount <= 2) return "guided";
  if (attemptCount <= 3) return "hints";
  if (attemptCount <= 4) return "explanation";
  return "direct";
}
...
export function formatTeachingResponse(
  ...
  const {
    maxQuestions = 2,
    includeHint = template.level !== "socratic",
    includeDocPointer = true,
  } = options;

  const lines: string[] = [template.opener, ""];
 ...
 */
export function getEncouragement(attemptCount: number, isCorrect: boolean): string {
  if (isCorrect) {
  ...
  const encouragementByAttempt: Record<number, string[]> = {
    1: [
